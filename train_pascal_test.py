...
def train(
    ...
    if load_decoder_path != None:
        ...
        print('done!')
    loss = np.zeros(5)
    loss_train, loss_test = np.zeros(5), np.zeros(5)
    epoch = 0.
    ...
    while epoch < training_epoch:
        ...
        sys.stdout.write(
            "kl:{:.4f}, shape:{:.4f}, reg:{:.4f}, pr:{:.4f}, rc:{:.4f}, c:{:.4f}, i:{:.4f} ".format(
                loss[0], loss[1], loss[2], loss[3], loss[4], loss_train[3], loss_train[4]))
        sys.stdout.write(
            "shape:{:.4f}, pr:{:.4f}, rc:{:.4f}, c:{:.4f}, i:{:.4f}  \r".format(
                loss_test[0], loss_test[1], loss_test[2], loss_test[3], loss_test[4]))
        sys.stdout.flush()
...
if __name__ == '__main__':
    sys.exit(train(
        ...
        config=config,
        save_path='./weights/pascal_test/',
        load_path='./weights/pascal_test/',
