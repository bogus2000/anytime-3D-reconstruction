...
def train(
    ...
    # np.save('./data/pascal/data_AE_dr/learn_' + learn + '/' + str(missing_pr) + '_cl_label.npy', category_labels)
    # del category_labels
    # output_images_gt = np.concatenate(output_images_gt, axis=0)
    # np.save('./data/pascal/data_AE_dr/learn_' + learn + '/' + str(missing_pr) + '_gt.npy', output_images_gt)
    # del output_images_gt
    # output_images_preds = np.concatenate(output_images_preds, axis=0)
    # np.save('./data/pascal/data_AE_dr/learn_' + learn + '/' + str(missing_pr) + '_pred.npy', output_images_preds)
...
if __name__ == '__main__':
    sys.exit(train(
        training_epoch=1000, learning_rate=1e-4,
        config=config,
        load_path='./weights/pascal_AE_dr/',
        missing_pr=0.9,
        learn='train',
    ))
