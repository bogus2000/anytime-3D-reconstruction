...
def train(
    ...
    while epoch < 1:
        ...
        output_images_pred, loss_shape, pr, rc = model.getEval(inputs=inputs_test, missing_prob=missing_pr)
    ...
    np.save('data_AE/learn_' + learn + '/' + str(missing_pr) + '_cl_label.npy', category_labels)
    del category_labels
    output_images_gt = np.array(output_images_gt)
    np.save('data_AE/learn_' + learn + '/' + str(missing_pr) + '_gt.npy', output_images_gt)
    del output_images_gt
    output_images_preds = np.array(output_images_preds)
    np.save('data_AE/learn_' + learn + '/' + str(missing_pr) + '_pred.npy', output_images_preds)
    del output_images_preds
    output_images_pred_correcteds = np.array(output_images_pred_correcteds)
    np.save('data_AE/learn_'+learn+'/' + str(missing_pr) + '_corrected.npy', output_images_pred_correcteds)
